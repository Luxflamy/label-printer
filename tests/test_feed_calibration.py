def test_build_auto_feed_gapdetect() -> None:
    ...
    assert "GAPDETECT 240,16" in text
    assert "FORMFEED" not in text
...
def test_build_auto_feed_can_formfeed_after_calibration() -> None:
    from label_printer.tspl.feed_calibration import build_auto_feed_calibration_text

    label = {"width_mm": 50, "height_mm": 30, "gap_mm": 2, "direction": 1}
    text = build_auto_feed_calibration_text(
        label,
        dots_per_mm=12,
        formfeed_after=True,
    )

    assert "GAPDETECT 360,24" in text
